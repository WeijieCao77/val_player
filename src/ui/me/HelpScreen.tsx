...
const SECTIONS: { title: string; lines: string[] }[] = [
  ...
  },
  {
    ...
    lines: [
      ...
      '坐替补想上场，就打跟队训练赛、发起对位挑战。新对位按你的岗位考三项不同的核心能力，手动和托管使用同一标准。以首发打满 8 场、教练信任到「信任」以上，或者以首发拿下冠军，就是他认定的首发。新比赛里，比赛失利且队内末位还要贡献不足，才累计表现不佳；连着三场会换下两周，刚以首发夺冠的保护场次不算。',
      '新比赛的贡献评分以击杀和伤害为主，助攻、死亡、首杀差和残局也算，不按位置白送分；教练评价、赛后排名和 MVP 使用同一评分。MVP 另有胜方加成，多图按实际总回合汇总，所以多半是胜方击杀和 ACS 最高的人。选手榜以本季评分为主，再按本季冠军和整场 MVP 加分。旧比赛记录与已颁奖项保留原判定。',
    ],
  },
  {
    title: '瓶颈和属性用词',
