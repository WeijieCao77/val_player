        ...
        <Panel
          ...
          actions={(
            <>
              ...
              <span className="tag t1" title={`上限是八项瓶颈按你的位置合起来的综合：破开任何一项，上限跟着涨。文字从低到高：${TIER_LADDER}。`}>
                {nums ? `综合 ${p.overall} / 上限 ${p.potential}` : `综合 ${attrWord(p.overall)}`}
              </span>
            </>
          )}
        >
          ...
          <div className="attr-row" style={{ marginTop: 8 }}>
            <span className="k" title="大场面里拿主意的成功率">心态</span><Bar value={me.mental} color="var(--accent)" /><span className="v">{nums ? Math.round(me.mental) : mentalWord(me.mental)}</span><span className="xp" />
          </div>
        ...
        <Panel title="现在">
          ...
          {nums ? (
            ...
            <p className="small" style={{ margin: 0 }}>
              士气 <b>{p.morale >= 70 ? '高昂' : p.morale >= 45 ? '平稳' : '低落'}</b>{team && <> · 教练 <b>{trustLabel(me.coachTrust)}</b></>} · 咖位 <b>{fanTier(me.fans).name}</b>
            </p>
          )}
          {me.tilt >= 55 && <p className="small" style={{ margin: '8px 0 0', color: 'var(--loss)' }}>心态有点绷不住：休息或赢一场能缓过来。</p>}
          {p.fatigue >= 45 && <p className="small" style={{ margin: '8px 0 0', color: 'var(--loss)' }}>{p.fatigue >= 70 ? '太累了：训练打折，容易受伤。' : '有点累：训练收益打折。'}</p>}
        </Panel>
        <Panel title="性格">
          ...
          }) : <p className="tiny faint" style={{ margin: '8px 0 0' }}>同一条轴选够五次，成为永久特质。</p>}
        </Panel>
      </div>
      <div>
          ...
          {recent.length === 0 ? <p className="muted" style={{ padding: 12, margin: 0 }}>还没打过比赛。</p> : (
            ...
            <div className="me-recent-wrap">
              <table className="me-recent">
                <thead><tr><th>赛事</th><th>对手</th><th>比分</th><th>K/D/A</th><th>评分</th><th></th></tr></thead>
                <tbody>
                  {recent.map((m) => (
                    <tr key={m.fixtureId}>
          ...
          <Panel title="杯赛" flush>
            ...
            <table><tbody>
              {me.pre.cups.slice().reverse().map((cu, i) => {
                // the cup as it was called that year, where I played it, and the round it ended in
