...
/**
 * The calendar, from my club's side. The career's own copy of the manager
 * game's schedule.
 *
 * 本队 carries more than my club's own fixtures: the rest of a playoff we are
 * out of (its final is still two days away), every international event
 * (greyed when we are not in it — it is still the week everyone watches), and
 * a placeholder row for the event we are booked into before its draw.
 */
...
export default function Schedule() {
  ...
  if (scope === 'all') {
    ...
    // calendar all the same: a side beaten in the upper final is in the lower
    // final two days on. Ours says 对手待定; a round that may or may not be
    // ours says 待定 vs 待定.
