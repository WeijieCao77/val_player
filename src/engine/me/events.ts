...
export const EVENTS: EventDef[] = [
  // ---- life
  { id: 'family_call', w: 8, max: 3, when: () => true, rec: 0,
  ...
  { id: 'old_friend', w: 6, max: 2, when: () => true, rec: 1,
  ...
  { id: 'airport', w: 5, max: 2, when: (s) => famous(s, 300), rec: 0,
  ...
  { id: 'insomnia', w: 7, max: 4, when: (s) => s.players[s.me!.id].fatigue >= 55, rec: 0,
  ...
  { id: 'locker_dinner', w: 7, max: 5, when: pro, rec: 0,
  ...
  { id: 'coach_talk', w: 6, max: 4, when: benched, rec: 0,
  ...
  { id: 'rookie_help', w: 5, max: 3, when: starter, rec: 0,
  ...
  // ---- the trade
  { id: 'ad', w: 5, max: 3, when: (s) => famous(s, 120), rec: 0,
  ...
  { id: 'caster', w: 4, max: 1, when: (s) => starter(s) && famous(s, 100), rec: 0,
    q: '解说在直播里给你起了个外号，弹幕刷疯了。', ctx: '外号不算好听，但很上口。',
    a: [{ t: '认了，改成 ID 后缀', g: 'show', e: { heat: 45, fans: 20 } }, { t: '不理', g: 'grind', e: { heat: 10 } }] },
  { id: 'variety', w: 3, max: 2, when: (s) => famous(s, 400) && pro(s), rec: 1,
  ...
  { id: 'stream_ladder', w: 4, max: 3, when: (s) => streams(s) && pre(s), rec: 0,
