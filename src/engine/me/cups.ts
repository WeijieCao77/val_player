...
export function enterCup(state: GameState, key: string, rng: Rng): string | null {
  ...
  pushLog(state, 'cup', `报名了${cup.name}${cup.fee ? `（${cny(cup.fee)}）` : ''}。抽到的队友：${me.pre.cup.mates.map((m) => `${m.ign}（${m.role}）`).join('、')}。`)
  return null
}
...
export function afterCupMatch(state: GameState, won: boolean, score: string, rng: Rng): CupRun | null {
  ...
  const line = rec.won
    ...
    : `${cup.name}止步${cup.rounds[Math.min(reached, cup.rounds.length - 1)].label}${prize ? `，奖金 ${cny(prize)}` : ''}。`
