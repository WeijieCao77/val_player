...
export function windowRuleLines(state: GameState): string[] {
  ...
  if (y <= 2022) {
    ...
    out.push('有了 VCT 联赛以后，VCT 联赛俱乐部和 Challengers 俱乐部一个规矩：俱乐部打的每一项赛事，从自己第一场比赛所在的那个阶段的第一天，锁名单到赛事最后一天——联赛的揭幕赛、第一赛段、第二赛段（2027 年起是杯赛、公开季后赛和公开资格赛），Challengers 联赛的各个赛段，大师赛、冠军赛、最后机会资格赛、晋级赛，打的海选、资格赛和升降级赛也算。直接进正赛的，前面那几周海选不跟着锁；从海选打上来的，从海选第一天就锁。提前出局、没打进季后赛，也要等这项赛事结束才解锁；只打了海选或资格赛、没打进正赛的，那一轮结束解锁。你的俱乐部报名打决胜局的，从那轮海选第一天锁，输了第二天解锁，赢了锁到赛事结束。')
