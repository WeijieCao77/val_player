...
export const ENDINGS_ME: EndingDef[] = [
  { key: 'breaker', title: '破局者', text: '同一年捧起 Masters 和 Champions。这个赛区的天花板，是你亲手推上去的。',
  ...
  { key: 'dynasty', title: '王朝', text: '两个 Champions。以后人们提起这个时代，会先提你的名字。', cond: (s) => intl(s, /Champions/).filter((t) => t.started).length >= 2 },
  { key: 'world', title: '世界冠军', text: '你站在了那个舞台的最中央。一次就够写进历史。', cond: (s) => intl(s, /Champions/).some((t) => t.started) },
  { key: 'master', title: '大师', text: 'Masters 冠军。离最高处只差一步，但你确实站上去过。', cond: (s) => intl(s, /Masters/).some((t) => t.started) },
  { key: 'uncrowned', title: '无冕之王', text: '打进过 Champions 决赛，输了。那一晚很多人记得你，没有奖杯记得你。', cond: (s) => !!s.me!.flags.champFinalLost },
