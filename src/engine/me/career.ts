...
export function createCareer(o: CareerOpts): GameState {
  ...
  if (o.start === 'pre') {
    ...
    pushLog(state, 'info', `没有队伍。${ladderLabel(me.pre.ladder)}，存款 $${me.money.toLocaleString()}。${cupFor(state, 'city')?.name}第 7 周报名、之后一周打一轮，${cupFor(state, 'premier')?.name}第 15 周报名，主播杯要粉丝过 ${fansCn(cupFor(state, 'streamer')?.minFans ?? 60)} 才请你。`)
  } else {
    me.ap = AP_SEASON
    me.apMax = AP_SEASON
    p.rounds = 400
