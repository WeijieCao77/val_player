...
export function createCareer(o: CareerOpts): GameState {
  ...
  if (o.start === 'pre') {
    ...
    pushLog(state, 'info', `没有队伍。${ladderLabel(state)}，存款 ${cny(me.money)}。${cupFor(state, 'city')?.name}在第 7 周开打，${cupFor(state, 'premier')?.name}在第 15 周，主播杯要粉丝过 ${fansCn(cupFor(state, 'streamer')?.minFans ?? 60)} 才请你。`)
  } else {
    me.ap = AP_SEASON
    me.apMax = AP_SEASON
    p.rounds = 400
