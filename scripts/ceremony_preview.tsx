 ...
 *
 *   npm run dev  →  /ceremony-preview.html?k=draw   (draw|depart|final|media|rehab|farewell)
 *
...
const about = q.get('about') ?? { draw: '2027 冠军赛', depart: '柏林', final: '冠军赛 总决赛', media: '第一赛段', rehab: '腱鞘炎复发', farewell: '2035 赛季' }[kind] ?? ''
    ...
    <GameCtx.Provider value={{
      ...
      <div style={{ padding: 20 }}>
        <p className="small">仪式 <b>{kind}</b> · 第 {n} 次提交 · 换一个：
          {(['draw', 'depart', 'final', 'media', 'rehab', 'farewell'] as CerKind[]).map((k) => (
