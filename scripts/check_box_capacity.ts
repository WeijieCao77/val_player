...
try {
  ...
  for (const fault of ['write', 'rename'] as const) {
    ...
    assert.equal(_boxState().length, 1)
    assert.deepEqual(_boxState()[0].votes, ['author', 'reader'], '合并重复票去重')
  ...
  assert.ok(counts().bytes > HARD)
  assert.deepEqual(_boxState(), hugeBefore.slice(2))
  assert.equal((await api('vote', { id: '00000002', vid: voters[0], on: false })).ok, true, '超过上限仍允许减少一票')
  assert.equal(_boxState()[0].votes.length, 4999)
